...
export default function Filter({ filter }: { filter: ListingFilter }) {
  const { values, expandable, isExpanded, toggleIsExpanded } = useFilter(filter);
  ...
  return (
    <div className="rounded-xl border bg-white p-5 shadow-lg">
      ...
      <div className="mt-3 max-h-[18rem] overflow-y-auto">
        {filter.type === "radio" && (
          <label className="flex items-center justify-start gap-3">
            <Input className="w-fit" type={filter.type} name={filter.name} value="All" />
            <span>All</span>
          </label>
        )}
        ...
        {values.map((val) => (
          <label key={val} className="flex items-center justify-start gap-3">
            <Input className="w-fit" type={filter.type} name={filter.name} value={val} />
            <span>{val}</span>
          </label>
        ))}
      </div>
      ...
      {expandable && (
        <Button onClick={toggleIsExpanded} size="sm" className="text-primary-default">
